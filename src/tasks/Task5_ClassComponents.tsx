    ...
    return (
      <div className="task-container">
        <div className="instruction-box">
          ...
          <ul>
            <li>Реализуйте классовый компонент с <code>this.state</code> и <code>this.setState</code>.</li>
            <li>Добавьте методы жизненного цикла: <code>componentDidMount</code>, <code>componentDidUpdate</code>, <code>componentWillUnmount</code>.</li>
            <li>Записывайте историю каждого изменения счетчика в массив <code>logs</code> внутри <code>componentDidUpdate</code>.</li>
          </ul>
        </div>
        ...
        <div className="demo-area">
          ...
          <div style={{ maxHeight: '150px', overflowY: 'auto', background: 'var(--bg-secondary)', padding: '0.75rem', borderRadius: '6px' }}>
            {this.state.logs.length === 0 ? (
              <p style={{ color: 'var(--text-subtle)', fontStyle: 'italic' }}>Журнал пуст. Нажмите кнопки +1 или -1.</p>
            ) : (
              <ul style={{ paddingLeft: '1.2rem', margin: 0 }}>
                {this.state.logs.map((log, index) => (
                  <li key={index} style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.2rem' }}>
                    {log}
                  </li>
                ))}
              </ul>
